...
app.put('/api/broadcast/:id/state', async (req, res) => {
    ...
    try {
        ...
        const newState = Object.assign(Object.assign({}, currentState), { position: position || 0, isPlaying: isPlaying !== undefined ? isPlaying : false, lastUpdated: lastUpdated || Date.now(), isActive: true, updatedAt: admin.firestore.FieldValue.serverTimestamp() });
        // Update Firestore
...
app.put('/api/broadcast/:id/song', async (req, res) => {
    ...
    try {
        ...
        await db.collection('broadcast_songs').doc(song.id).set(Object.assign(Object.assign({}, song), { broadcastId, updatedAt: admin.firestore.FieldValue.serverTimestamp() }));
        // Store in memory
        songs.set(song.id, song);
        // Update broadcast state with current song
        const currentState = broadcastStates.get(broadcastId) || {};
        const newState = Object.assign(Object.assign({}, currentState), { currentSong: song.id, position: 0, isPlaying: true, duration: song.duration || 0, lastUpdated: Date.now(), isActive: true, updatedAt: admin.firestore.FieldValue.serverTimestamp() });
...
exports.broadcastApi = functions.https.onRequest(app);
