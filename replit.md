...
## Recent Changes
- **Bluetooth MIDI Support for All Platforms (Sep 21, 2025)**: Expanded Bluetooth MIDI support from Android-only to all platforms (Chrome/Edge desktop and mobile). Implemented two-tab device manager interface separating regular MIDI devices from Bluetooth MIDI devices. Added auto-reconnect functionality with localStorage persistence for regular MIDI devices. Bluetooth devices now show "BLE" suffix for clear identification. Web Bluetooth API support properly detected with fallback messaging for unsupported browsers (Firefox/Safari).
