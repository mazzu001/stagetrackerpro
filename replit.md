...
## Recent Changes
- **Complete MIDI System Rebuild (Sep 22, 2025)**: Rebuilt entire MIDI system from scratch after total failure. Removed 1,800+ lines of broken code across 11 files that caused application freezing. New simple architecture with shared context pattern (SimpleMidiContext) ensures single MIDI instance across all components. MIDI only initializes on user button click, preventing all blocking operations. Features simple device dialog with "Connect MIDI Devices" button for professional users, visual command indicators in lyrics (Music icon), and standardized [[TYPE:VALUE:CHANNEL]] format. System now works without timeouts or freezing.
