...
## Recent Changes
- **Non-blocking MIDI Initialization (Sep 21, 2025)**: Fixed critical app startup freeze caused by MIDI initialization blocking main JavaScript thread for 5-30+ seconds. Implemented proper async initialization pattern with loading states and timeout protection. Changes: Reduced timeout from 30s to 5s, added isInitializing state tracking, made initializeMidi properly non-blocking, and updated device manager UI to show immediately with loading indicator. MIDI now initializes only on user interaction without freezing the UI. Device manager opens instantly with clear loading feedback.
