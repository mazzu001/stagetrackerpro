...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post('/api/subscription/cancel', isAuthenticated, async (req: any, res) => {
    try {
      ...
      console.log(`📅 Subscription ${subscription.id} set to cancel at period end for user ${user.email}`);

      res.json({ success: true, subscription: { cancelAtPeriodEnd: true } });
    } catch (error: any) {
  ...
  app.post('/api/subscription/reactivate', isAuthenticated, async (req: any, res) => {
    try {
      ...
      console.log(`✅ Subscription ${subscription.id} reactivated for user ${user.email}`);

      res.json({ success: true, subscription: { cancelAtPeriodEnd: false } });
    } catch (error: any) {
  ...
  app.post('/api/subscription/portal', isAuthenticated, async (req: any, res) => {
    try {
      ...
      const session = await stripe.billingPortal.sessions.create({
        customer: user.stripeCustomerId,
        return_url: `${req.headers.origin || 'http://localhost:5000'}/subscription-management`,
      });
