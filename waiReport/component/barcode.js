(function() {
    var YRCompBase = {
        ...
        properties : {
            'className' : {'type': typeof(''), visible:true, readonly:false, defaultValue: ''},
            'dataField'     : {'type': typeof(''), visible: true, readonly: false, defaultValue: ''},
            'backColor'     : {'type': 'color',  visible:true, readonly:false, defaultValue: '', link:'style.bgColor'},
            'barColor'     : {'type': 'color',  visible:true, readonly:false, defaultValue: '', link:'style.Color'},
            'value'  : {'type': typeof(''), visible:true, readonly:false, defaultValue: ''},
            'formatType': {'type': 'select', visible:true, readonly:false, defaultValue: '', values: [ {d:'codabar',v:'codabar'}, {d:'std25',v:'std25'}, {d:'int25',v:'int25'},
                {d:'ean8',v:'ean8'}, {d:'ean13',v:'ean13'}, {d:'code11',v:'code11'}, {d:'code39',v:'code39'}, {d:'code93',v:'code93'}, {d:'code128',v:'code128'}]}
            /*'barcodeWidth'  : {'type': typeof(''), visible:false, readonly:false, defaultValue: ''}*/
        },
        initialize : function() {
        ...
        execDraw   : function(aCtx, aData) {
            ...
            var barcode = {
                settings:{
                    ...
                    moduleSize: 5,
                    //showHRI: true,
                    showHRI: false, // canvas에서 폰트가 잘 안먹힘
                    addQuietZone: true,
                    marginHRI: 5,
                    bgColor: "#FFFFFF",
                    color: "#000000",
                ...
                digitToCanvasRenderer : function($container, settings, digit, hri, xi, yi, mw, mh){
                    ...
                    var lines = digit.length;
                    var columns = digit[0].length;
                    //console.log(digit, "어캐 가져오지 ㅣ걸")
                    //test
                    //YRCompBase.properties.barcodeWidth.barcodeW = columns;
                    //YRCompBase.properties.barcodeWidth = columns;
                    barcodeWidth = columns;
                    //
                    ...
                    if (settings.showHRI){
                        ...
                        var fontStyle = fontSize + " " + fontName;
            ...
            var barcodeValue = (this.value !== undefined) ? this.value : '';
            if (this.dataField !== undefined) {
                if (aData !== undefined) {
                    if (aData[this.dataField] !== undefined) {
                        if(this.value !== aData[this.dataField]) {
                            barcodeValue = aData[this.dataField];
                            this.value = barcodeValue;
                        }
                    }
                }
            }
            //console.log("barcode:", barcodeValue, "//aData:", aData, "//this.value:", this.value);
            //var barcodeValue = (this.value !== undefined) ? this.value : '';

            var value = barcodeValue;
            var btype = this.formatType;
            var renderer = "canvas";
            var self = this;
            var mg = 2;
            var settings = {
                ...
                fontSize: parseInt(self.Font.Size),
                bgColor: self.backColor,
                color: self.barColor,
                barWidth: 1,
                barHeight: self.height - (mg * 2),
                moduleSize: $("#moduleSize").val(),
                posX: self.left + mg, //$("#posX").val(),
                posY: self.top + mg, //$("#posY").val(),
                addQuietZone: false //$("#quietZoneSize").val()
            };

            if(value != undefined && value != "") {
                $(aCtx.canvas).hide().barcode(value, btype, settings);
                settings.barWidth = parseInt(self.width / barcodeWidth);
                settings.bgColor = self.backColor;
                settings.color = self.barColor;
                //fontSize = parseInt(self.Font.Size),
                //console.log("설정된 넓이 안에 줄 1개당 들어가지는 넓이", settings.barWidth, "설정한 넓이값:", self.width, "줄수", barcodeWidth);
                $(aCtx.canvas).show().barcode(value, btype, settings);
            }
            self.updateCanvas({});
