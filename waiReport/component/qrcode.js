(function(){
    var YRCompBase = {
        ...
        properties : {
            //'data'              : {'type': 'image',    visible:false, readonly:false, defaultValue: ''},
            //'backStyle'         : {'type': 'select',   visible:true, readonly:false, defaultValue: '', values:$waiReport.const.defPropSelectValues.fillStyle},
            //'backColor'         : {'type': 'color',    visible:true, readonly:false, defaultValue: ''},
            //'lineStyle'         : {'type': 'select',   visible:false, readonly:false, defaultValue: '', values:$waiReport.const.defPropSelectValues.lineStyle},
            //'lineWeight'        : {'type': 'strInt',   visible:false, readonly:false, defaultValue: '1'},
            //'lineColor'         : {'type': 'color',    visible:false, readonly:false, defaultValue: 'black'},
            //'pictureAlignment'  : {'type': 'select',   visible:false, readonly:false, defaultValue: '', values:$waiReport.const.defPropSelectValues.imgAlign},
            'dataField'     : {'type': typeof(''), visible: true, readonly: false, defaultValue: ''},
            'sizeMode'      : {'type': 'select',   visible:false, readonly:false, defaultValue: '', values:$waiReport.const.defPropSelectValues.imgSize},
            'value'  : {'type': typeof(''), visible:true, readonly:false, defaultValue: ''},
            //'size'  : {'type': typeof(''), visible:true, readonly:false, defaultValue: ''}
        },
        initialize : function() {
            ...
            //alert('loadedImage = true');
            this.loadedImage = true;

            var self = this;

            if(self.type === "qrcode"){
                //self.propertyNames.width.info.visible = false;
                self.propertyNames.height.info.visible = false;
            }
        ...
        execDraw   : function(aCtx, aData) {
            ...
            (function (global, factory) {
                ...
                var QRious$1 = function () {
                    ...
                    function QRious(options) {
                        ...
                        //YRCompBase.properties.data.src = this.image;
                        //YRCompBase.properties.data = this.image.src;
                        //YRCompBase.properties.barcodeWidth.barcodeW = columns;
                        this._renderers = [new CanvasRenderer(this), new ImageRenderer(this)];
                        this.update();
                        //YRCompBase.properties.data = this.image.src;
                        dataSrc = this.image.src;
                    }
                    _createClass(QRious, [{
                        key: 'toDataURL',
                    ...
                    /*QRious.DEFAULTS.element = $(aCtx.canvas);
                     QRious.DEFAULTS.value = 'https://neocotic.com/qrious';*/
                    return QRious;
                }();
            ...
            if (this.dataField !== undefined) {
                if (aData !== undefined) {
                    if (aData[this.dataField] !== undefined) {
                        if(this.value !== aData[this.dataField]) {
                            this.value = aData[this.dataField];
                        }
                    }
                }
            }
            ...
            var lineStyle = self.lineStyle;
            var lineWidth = Number(self.lineWeight);
            var lineColor = $waiReport.rtl.util.int2color( self.lineColor );

            var bgStyle = (this.backStyle===undefined)?'0':this.backStyle;
            var bgColor = $waiReport.rtl.util.int2color( self.backColor );
            //console.log('['+this.name+'] : ',self.backColor);
            var lStyleDashes = {
                '1':[],
                '2':[6,3],
                '3':[2,2],
            ...
            aCtx.save();
            if ((lineStyle !== undefined)&&(lineStyle !== '0')){
                aCtx.setLineDash(lStyleDashes[lineStyle]);
                aCtx.lineWidth  = (lineWidth===NaN)? 1 : lineWidth;
                aCtx.strokeStyle = (lineColor==='') ? '#000000' : lineColor;
            } else {
                aCtx.setLineDash([]);
                aCtx.lineWidth  = 1;
                aCtx.strokeStyle = 'rgba(0,0,0,0)';
            }

            var r = {l:left,t:top,w:width,h:height};

            aCtx.beginPath();
            aCtx.setLineDash([0]);
            aCtx.moveTo(r.l, r.t);
            aCtx.lineTo(r.l+r.w, r.t);
            aCtx.lineTo(r.l+r.w, r.t+r.h);
            aCtx.lineTo(r.l, r.t+r.h);
            aCtx.lineTo(r.l, r.t);
            aCtx.closePath();
            if (bgStyle == '1') {
                aCtx.fillStyle = (bgColor==='') ? 'rgba(0,0,0,0)' : bgColor;
                aCtx.fill();
            }
            aCtx.stroke();
            aCtx.restore();

            var ix = left,iy = top,iw = 0, ih=0;

            //var dataSrc = YRCompBase.properties.data;
            if (dataSrc !== undefined){
                if (dataSrc.trim()!==''){
                    this.image.src = dataSrc;
                }
            }

            var picAlign = parseInt(((this.pictureAlignment===undefined)?'2':this.pictureAlignment).trim());
            var picSize  = (this.sizeMode===undefined)?'0':this.sizeMode;



            function imageProc() {
                var sx=0,sy=0,sw=self.image.width,sh=self.image.height;
                var dx=left,dy=top,dw=self.image.width,dh=self.image.height;

                if (picSize == '0') {
                    switch(picAlign){
                        case (0): // top-left Align
                            break;
                        ...
                        case (2): // center align
                            if (sw > width) {
                                sx=sx+Math.floor((sw-width)/2);
                            } else {
                                dx = left+Math.floor((width-sw)/2);
                            }
                            if (sh > height) {
                                sy=sy+Math.floor((sh-height)/2);
                            } else {
                                dy = top+Math.floor((height-sh)/2);
                            }
                            break;
                    ...
                    if (sw > width)  { sw = width;  dw = width;  }
                    if (sh > height) { sh = height; dh = height; }
                    aCtx.drawImage(self.image,sx,sy,sw,sh,dx,dy,dw,dh);
                } else
                if (picSize == '1') {
                    aCtx.drawImage(self.image,sx,sy,sw,sh,left,top,width,height);
                } else
                if (picSize == '2') {
                    var hRatio = width  / sw;
                    var vRatio = height / sh;
                    var ratio  = Math.min ( hRatio, vRatio );
                    var centerShift_x = ( width - sw*ratio ) / 2;
                    var centerShift_y = ( height - sh*ratio ) / 2;
                    // console.log('Width',width,sw,hRatio,centerShift_x);
                    // console.log('Height',height,sh,vRatio,centerShift_y);
                    ...
                    aCtx.drawImage(self.image,sx,sy,sw,sh,left+centerShift_x,top+centerShift_y,sw*ratio, sh*ratio);
                }

                self.updateCanvas({});
            }

            if (this.loadedImage !== true) {
                this.image.onload = function(){
                    if ( self.loadedImage !== true ) imageProc();
                };
                this.image.onerror = function(){
                    ...
                    aCtx.font = (Math.min(width,height)-(Math.min(width,height)/1.8))+'px Tahoma';
                    aCtx.fillText('Not image-data for web',left+(width/2),top+(height/2),width);
